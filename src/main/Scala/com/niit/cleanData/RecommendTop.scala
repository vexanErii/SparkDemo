...
class RecommendTop {
  def run(): Unit ={
    ...
    val processedDF= spark.sql("select id,novel_name,novel_read,novel_recommend from jobinfo order by novel_recommend desc limit 10")
    // 将处理后的数据写回到MySQL
    processedDF.write.format("jdbc")
      .option("url", "jdbc:mysql://192.168.56.104:3306/novel?useSSL=false")
      .option("dbtable", "recommendtop")
      .option("user", "root")
      .option("password", "root")
