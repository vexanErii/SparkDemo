...
class TypeTopRead {
  def run(): Unit = {
    ...
    val processedDF = spark.sql("WITH RankedNovels AS (      SELECT           id,           novel_name,           novel_read,           novel_type,          ROW_NUMBER() OVER (PARTITION BY novel_type ORDER BY novel_read DESC) AS rn      FROM jobinfo  )  SELECT       id,       novel_name,       novel_read,       novel_type  FROM RankedNovels  WHERE rn = 1")
    // 将处理后的数据写回到MySQL
    processedDF.write.format("jdbc")
      .option("url", "jdbc:mysql://192.168.56.104:3306/novel?useSSL=false")
      .option("dbtable", "typeTopRead")
      .option("user", "root")
      .option("password", "root")
